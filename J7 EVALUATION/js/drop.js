...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const booked = outing.reservations.length

  // N-07 : compteur places figé après 1re visite (cache session)
  const placesCacheKey = `drop-places-${id}`
  let displayedPlacesLeft = outing.placesLeft
  const cachedPlaces = sessionStorage.getItem(placesCacheKey)
  if (cachedPlaces !== null) {
    displayedPlacesLeft = Number(cachedPlaces)
  } else {
    sessionStorage.setItem(placesCacheKey, String(outing.placesLeft))
  }

  const noPlacesLeft = displayedPlacesLeft <= 0
  ...
  container.innerHTML = `
    ...
    <article class="card drop-live">
      ...
      <div class="places-block">
        ...
        <p class="places-left"><strong id="places-left">${displayedPlacesLeft}</strong> / ${outing.maxPlaces}</p>
